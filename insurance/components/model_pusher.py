...
class ModelPusher:
    ...
    def initiate_model_pusher(self,)->ModelPusherArtifact:
        try:
            ...
            feature_encoder = load_object(file_path=self.data_transformation_artifact.feature_encoder_path)

            #model pusher dir
            logging.info(f"Saving model into model pusher directory")
            save_object(file_path=self.model_pusher_config.pusher_transformer_path, obj=transformer)
            save_object(file_path=self.model_pusher_config.pusher_model_path, obj=model)
            save_object(file_path=self.model_pusher_config.pusher_feature_encoder_path, obj=feature_encoder)
            ...
            feature_encoder_path=self.model_resolver.get_latest_save_feature_encoder_path()

            save_object(file_path=transformer_path, obj=transformer)
            save_object(file_path=model_path, obj=model)
            save_object(file_path=feature_encoder_path, obj=feature_encoder)
